...
def map_gene_names(genes,use_col='MGI ID',path_genewalk=None,
                      path_anno='/storage/groups/ml01/workspace/karin.hrovatin/data/pancreas/gene_lists/genomeAnno_ORGmus_musculus_V103_SSFltp_2y__MUC13974.tsv'
                     ):
    """
...
def opt_order_withincl(X:pd.DataFrame,clusters:pd.Series,cl_order:list=None,
    ...
    genes_dict=X.groupby('cl').apply(
                 ...
                 lambda x: x.index.values[opt_order(x.drop('cl',axis=1))]).to_dict()
